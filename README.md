...
- **Phase 6 + 6.1 (done):** one unified "This year" reading in the paid tier. Under the hood, four separately verified calculations — the annual pillar (Lìchūn-exact, same mechanism as the birth chart) with its favorable/unfavorable Ten God role, clash (冲, cross-verified against the calculation library), harm (害, the six documented pairs, copy kept deliberately generic), and Blood Blade (血刃, month-branch derived, framed as practical care rather than warning). Delivered as a single narrative led by which life domain the year leans into (year=family/reputation, month=career, day=self/partner, hour=children/aspirations), with one collapsed mechanics note underneath. Flying Blade, combination, and punishment deliberately excluded pending verification.
- **Access gate (current):** a light shared-passphrase screen (passphrase configured via environment variable, never stored in this repository) holds back general access while the privacy policy, terms of service, and disclaimer are drafted. The app collects real birth details and holds live accounts, so it stays gated until those documents are in place. It guards the app only — per-user data is protected independently by database row-level security.
